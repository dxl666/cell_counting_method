...
class SCTransUNet(nn.Module):
    def __init__(self, config, img_size=224, num_classes=2, zero_head=False, vis=False):
        super(SCTransUNet, self).__init__()
        out_channels = [2 ** (i + 6) for i in range(4)]
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, vis)
        self.msff1 = MSFF(out_channels[0], out_channels[1])
        self.msff2 = MSFF(out_channels[2], out_channels[3])
    ...
    def forward(self, x):
        ...
        new_skip_features.append(x1)
        new_skip_features.append(x2)
        decoder_feature = self.decoder(new_skip_features, decoder_feature)
